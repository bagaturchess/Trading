...
public class Player_OpenClose_ManyPos_3AVG extends Player_BuySellImpl<OpenCloseMetainf> {
	...
	public Player_OpenClose_ManyPos_3AVG(String _name, int toCurrencyType, IBroker _broker, int _avgLength1, int _avgLength2, int _avgLength3) {
		super(_name + "[" + _avgLength1 + ", " + _avgLength2 + ", " + _avgLength3 + "]", toCurrencyType, _broker, -1);
		...
		if (_avgLength3 < 1) {
			throw new IllegalStateException("_avgLength3=" + _avgLength3);
		}
		...
		if (_avgLength3 <= _avgLength2) {
			throw new IllegalStateException("_avgLength2=" + _avgLength2 + ", _avgLength3=" + _avgLength3);
		}
		
		avgLength1 = _avgLength1;
		avgLength2 = _avgLength2;
		avgLength3 = _avgLength3;
	}
	...
	public void addNewRate(IAccount account, int order, double rate) { 
		...
		avgs3 = SequenceUtils.buildAVGSequence(sequence, avgLength3);
		...
		//Decision
		if (avgs3[lastPos - 1] <= avgs3[lastPos - 2]) {
